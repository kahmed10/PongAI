...
def run_selected_action(a_t, s_t, game_state):
    ...
    s_t1 = np.stack((x_t1, s_t[:,:,1], s_t[:,:,2], s_t[:,:,3]), axis=2)
...
def compute_target_q(r_batch, readout_j1_batch, terminal_batch):
    ...
    for i in range(0, len(terminal_batch)):
        ...
        if terminal_batch[i] is True:
            target_q_batch.append(r_batch[i])
        else:
            target_q = max(readout_j1_batch[i])
            target_q = r_batch[i] + GAMMA*target_q
            target_q_batch.append(min(target_q,10000000.))
